...
generate_type = {'iv_s':['train', 'data/in_vocab.subset.tro.37'],
                ...
                'custom':['custom', 'data/custom_text.txt']}
...
class IAMDataset(Dataset):
    ...
    def get_style_ref(self, wr_id):
        ...
        laplace_images = []
        for index in style_index:
            laplace_path = os.path.join(self.laplace_path, wr_id, style_list[index])
            laplace_image = cv2.imread(laplace_path, flags=0)
            if laplace_image is None:
                # Create dummy laplace image (zeros) with same shape as style image
                laplace_image = np.zeros_like(style_images[0])
            laplace_images.append(laplace_image/255.0)
        ...
        '''laplace images'''
            # laplace_images are already processed in the loop above
...
class Random_StyleIAMDataset(IAMDataset):
    ...
    def get_style_ref(self, wr_id): # Choose the style image whose length exceeds 32 pixels
        ...
        for index in range(len(style_list)):
            ...
            if laplace_image is None:
                # Create dummy laplace image if missing
                laplace_image = np.zeros_like(style_image)
            
            # BEST PREPROCESSING PIPELINE FOR STYLE TRANSFER
            
            # 1. Noise reduction with bilateral filter (preserves edges)
            style_image = cv2.bilateralFilter(style_image, 9, 75, 75)
            
            # 2. Adaptive histogram equalization for better contrast
            clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8,8))
            style_image = clahe.apply(style_image)
            
            # 3. Edge enhancement using unsharp masking
            gaussian = cv2.GaussianBlur(style_image, (0,0), 2.0)
            style_image = cv2.addWeighted(style_image, 1.5, gaussian, -0.5, 0)
            
            # 4. Adaptive thresholding to make text more distinct
            style_image = cv2.adaptiveThreshold(style_image, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11, 2)
            
            # 5. Morphological operations to clean up text
            kernel = np.ones((2,2), np.uint8)
            style_image = cv2.morphologyEx(style_image, cv2.MORPH_CLOSE, kernel)
            
            # 6. Resize to standard dimensions (height=64, width=352)
            style_image = cv2.resize(style_image, (352, 64), interpolation=cv2.INTER_CUBIC)
            laplace_image = cv2.resize(laplace_image, (352, 64), interpolation=cv2.INTER_CUBIC)
            
            # 7. Final smoothing for better feature extraction
            style_image = cv2.GaussianBlur(style_image, (3, 3), 0.5)
