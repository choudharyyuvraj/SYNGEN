...
class IAMDataset5Percent(IAMDataset):
    ...
    def __init__(self, image_path, style_path, laplace_path, type, content_type='unifont', max_len=9, subset_ratio=0.005):
...
def main(opt):
    ...
    print('Training on the following images:')
    with open('used_training_images.txt', 'w') as f:
        for idx in train_dataset.indices:
            entry = train_dataset.data_dict[idx]
            print(entry['image'])
            f.write(entry['image'] + '\n')
