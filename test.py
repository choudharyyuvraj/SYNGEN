...
def main(opt):
    ...
    # Set up distributed only if using multiple GPUs

    if opt.device == 'cuda' and torch.cuda.is_available() and torch.cuda.device_count() > 1:
    ...
    if len(texts) % totol_process == 0:
        temp_texts = texts[rank*each_process:(rank+1)*each_process]
    else:
        each_process += 1
        temp_texts = texts[rank*each_process:(rank+1)*each_process]
    ...
    for x_text in tqdm(temp_texts, position=0, desc='batch_number'):
        ...
        for (data_val, laplace, wid) in data_loader:
            ...
            print(f"  style_input: {style_input.shape}")
            print(f"  laplace: {laplace.shape}")
            print(f"  text_ref: {text_ref.shape}")
            print(f"  x: {x.shape}")
